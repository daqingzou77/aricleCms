...
(haoutil.version = '2.3'),
  ...
  (haoutil.loading = {
      ...
      this.close(),
        window.NProgress
          ? ((t = t || {}).color
            ...
            NProgress.start())
          : window.layer && (this.index = layer.load(2, { shade: [0.3, '#000000'] }));
    },
    hide: function() {
      this.close();
    },
    close: function() {
      window.NProgress
        ? NProgress.done(!0)
        : window.layer && (-1 != this.index && layer.close(this.index), (this.index = -1));
    },
  }),
  window.noArrayPrototype ||
    ...
    /(E+)/.test(t) &&
      (t = t.replace(
        ...
        (1 < RegExp.$1.length ? (2 < RegExp.$1.length ? '星期' : '周') : '') +
          { 0: '日', 1: '一', 2: '二', 3: '三', 4: '四', 5: '五', 6: '六' }[this.getDay() + '']
      )),
    e))
      new RegExp('(' + n + ')').test(t) &&
        (t = t.replace(
          RegExp.$1,
          1 == RegExp.$1.length ? e[n] : ('00' + e[n]).substr(('' + e[n]).length)
        ));
    return t;
  }),
  (haoutil.color = {
    random: function() {
      return (
        '#' +
        (function(t) {
          return (t += '0123456789abcdef'[Math.floor(16 * Math.random())]) && 6 == t.length
            ? t
            : arguments.callee(t);
        })('')
      );
      ...
      formatTime: function(t) {
        return (
          ...
          t < 60
            ? t.toFixed(0) + '秒'
            : 60 <= t && t < 3600
            ? Math.floor(t / 60) + '分钟' + Math.floor(t % 60) + '秒'
            : ((t = Math.floor(t / 60)), Math.floor(t / 60) + '小时' + Math.floor(t % 60) + '分钟')
        );
      },
      base64: function(t) {
        return (function(t) {
          for (
            var e = '',
              n = t.length % 6,
              r = t.substr(0, t.length - n),
              o = t.substr(t.length - n, n),
              i = 0;
            i < r.length;
            i += 6
          ) {
  ...
  (haoutil.system = (function() {
    function n(t, e) {
      ...
      (n.rel = 'stylesheet'), (n.async = e), (n.href = t), o.appendChild(n);
    }
    function r(t, e) {
      var n = document.createElement('script');
      (n.charset = 'utf-8'), (n.async = e), (n.src = t), o.appendChild(n);
    }
    ...
    return {
      ...
      getExplorerInfo: function() {
        ...
        return 0 <= t.indexOf('msie')
          ...
          : 0 <= t.indexOf('chrome')
          ? { type: 'Chrome', version: Number(t.match(/chrome\/([\d]+)/)[1]) }
          : 0 <= t.indexOf('opera')
          ? { type: 'Opera', version: Number(t.match(/opera.([\d]+)/)[1]) }
          : 0 <= t.indexOf('Safari')
          ? { type: 'Safari', version: Number(t.match(/version\/([\d]+)/)[1]) }
          : { type: t, version: -1 };
      },
