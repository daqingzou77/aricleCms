...
GMap.SVG.Tile = GMap.SVG.extend({

	initialize: function (tileCoord, tileSize, options) {
		GMap.SVG.prototype.initialize.call(this, options);
	...
	addTo: function(map) {
		...
		if (this.options.interactive) {
			for (var i in this._layers) {
				...
				// By default, GMap tiles do not have pointer events.
				layer._path.style.pointerEvents = 'auto';
				this._map._targets[GMap.stamp(layer._path)] = layer;
			}
		}
	},
	...
	removeFrom: function (map) {
		if (this.options.interactive) {
			for (var i in this._layers) {
				var layer = this._layers[i];
				delete this._map._targets[GMap.stamp(layer._path)];
			}
		}
		delete this._map;
	},
	...
	_initContainer: function() {
		GMap.SVG.prototype._initContainer.call(this);
		var rect =  GMap.SVG.create('rect');
	},
	...
	_addPath: function (layer) {
		...
		this._layers[GMap.stamp(layer)] = layer;
	},

	_updateIcon: function (layer) {
		var path = layer._path = GMap.SVG.create('image'),
		    icon = layer.options.icon,
		    options = icon.options,
		    size = GMap.point(options.iconSize),
...
GMap.svg.tile = function(tileCoord, tileSize, opts){
	return new GMap.SVG.Tile(tileCoord, tileSize, opts);
};

// 🍂class Symbolizer
// 🍂inherits Class
// The abstract Symbolizer class is mostly equivalent in concept to a `GMap.Path` - it's an interface for
...
var Symbolizer = GMap.Class.extend({
	...
	// `GMap.Path` options.  Internal use only.
	...
	// 🍂method render(renderer, style)
	// Updates the `GMap.Path` options used to style this symbolizer, and re-renders it.
	// Internal use only.
	...
	_clickTolerance: GMap.Path.prototype._clickTolerance,
});
	...
	_makeFeatureParts: function(feat, pxPerExtent) {
		...
		for (var i = 0; i < rings.length; i++) {
			...
			for (var j = 0; j < ring.length; j++) {
				...
				// Geojson-vt returns [,]
				part.push(GMap.point(coord).scaleBy(pxPerExtent));
			}
...
var PointSymbolizer = GMap.CircleMarker.extend({
	...
	render: function(renderer, style) {
		...
		this._radius = style.radius || GMap.CircleMarker.prototype.options.radius;
		this._updatePath();
	},
	...
	_makeFeatureParts: function(feat, pxPerExtent) {
		...
		if (typeof coord[0] === 'object' && 'x' in coord[0]) {
			// Protobuf vector tiles return [{x: , y:}]
			this._point = GMap.point(coord[0]).scaleBy(pxPerExtent);
			this._empty = GMap.Util.falseFn;
		} else {
			// Geojson-vt returns [,]
			this._point = GMap.point(coord).scaleBy(pxPerExtent);
			this._empty = GMap.Util.falseFn;
		}
	},
	...
	_updateBounds: function() {
		...
		if (icon) {
			var size = GMap.point(icon.options.iconSize),
			    anchor = icon.options.iconAnchor ||
			             size && size.divideBy(2, true),
			    p = this._point.subtract(anchor);
			this._pxBounds = new GMap.Bounds(p, p.add(icon.options.iconSize));
		} else {
			GMap.CircleMarker.prototype._updateBounds.call(this);
		}
	},
	...
	_updatePath: function() {
		if (this.options.icon) {
			...
			GMap.CircleMarker.prototype._updatePath.call(this);
		}
	},
	...
	_containsPoint: function(p) {
		...
		if (icon) {
			...
			return GMap.CircleMarker.prototype._containsPoint.call(this, p);
		}
	}
});
...
var LineSymbolizer = GMap.Polyline.extend({
...
var FillSymbolizer = GMap.Polygon.extend({
	includes: [Symbolizer.prototype, PolyBase],
 ...
 * they are.
 *
 * Extends GMap's `GMap.GridLayer`.
 */

GMap.VectorGrid = GMap.GridLayer.extend({

	options: {
		// 🍂option rendererFactory = GMap.svg.tile
		// A factory method which will be used to instantiate the per-tile renderers.
		rendererFactory: GMap.svg.tile,
	...
	initialize: function(options) {
		GMap.setOptions(this, options);
		GMap.GridLayer.prototype.initialize.apply(this, arguments);
		...
		vectorTilePromise.then( function renderTile(vectorTile) {
			...
			if (vectorTile.layers && vectorTile.layers.length !== 0) {
				...
				for (var layerName in vectorTile.layers) {
					...
					// add by wusheng
					// 图层过滤
					if ( !this.options.layerFilter(layer) ){
						continue;
					}
					...
					for (var i = 0; i < layer.features.length; i++) {
						...
						for (var j = 0; j < styleOptions.length; j++) {
							var style = GMap.extend({}, GMap.Path.prototype.options, styleOptions[j]);
			...
			GMap.Util.requestAnimFrame(done.bind(coords, null, null));
	...
	// 🍂method setFeatureStyle(id: Number, layerStyle: GMap.Path Options): this
	...
	// 图层更新时，同时更新标注层
    _update: function() {
        GMap.GridLayer.prototype._update.call(this);
        for(var tkey in this._vectorTiles) {
            var tile = this._vectorTiles[tkey];
            if (tile._tileCoord.z == this._tileZoom) {
                var layers = tile._layers;
                for(var lkey in layers) {
                    this._map.updateLabel(layers[lkey]);
                }
            }
        }
    },
 ...
 *
 */
GMap.vectorGrid = function (options) {
	return new GMap.VectorGrid(options);
};
 ...
 * `GMap.TileLayer`s. The difference is that the template must point to vector tiles
 ...
 *
 * ```
 * GMap.vectorGrid.protobuf("https://free-{s}.tilehosting.com/data/v3/{z}/{x}/{y}.pbf.pict?key={key}", {
 * 	vectorTileLayerStyles: { ... },
 * 	subdomains: "0123",
 ...
 *
 * ```
 * GMap.vectorGrid.protobuf("https://{s}.tiles.mapbox.com/v4/mapbox.mapbox-streets-v6/{z}/{x}/{y}.vector.pbf?access_token={token}", {
 * 	vectorTileLayerStyles: { ... },
 * 	subdomains: "abcd",
 ...
 * ```
 */
GMap.VectorGrid.Protobuf = GMap.VectorGrid.extend({

	options: {
		// 🍂section
		// As with `GMap.TileLayer`, the URL template might contain a reference to
		// any option (see the example above and note the `{key}` or `token` in the URL
		// template, and the corresponding option).
		//
		// 🍂option subdomains: String = 'abc'
		// Akin to the `subdomains` option for `GMap.TileLayer`.
		subdomains: 'abc',	// Like GMap.TileLayer
		//
...
// 		this._slicer = geojsonvt(geojson, options);
		this._url = url;
		GMap.VectorGrid.prototype.initialize.call(this, options);
	},
	...
	_getSubdomain: GMap.TileLayer.prototype._getSubdomain,
...
// 			z: this._getZoomForUrl()	/// TODO: Maybe replicate TileLayer's maxNativeZoom
		...
		if (this._map && !this._map.options.crs.infinite) {
			...
			if (this.options.tms) { // Should this option be available in GMap.VectorGrid?
				data['y'] = invertedY;
			}
			data['-y'] = invertedY;
		}
		...
		var tileUrl = GMap.Util.template(this._url, GMap.extend(data, this.options));
...
// 🍂factory GMap.vectorGrid.protobuf(url: String, options)
// Instantiates a new protobuf VectorGrid with the given URL template and options
GMap.vectorGrid.protobuf = function (url, options) {
	return new GMap.VectorGrid.Protobuf(url, options);
};
 ...
 * };
 *
 * GMap.vectorGrid.slicer(geoJsonDocument, {
 * 	vectorTileLayerStyles: {
 * 		sliced: { ... }
 * 	}
 * }).addTo(map);
 ...
 * ```js
 * var layer = GMap.vectorGrid.slicer(topojson, options);
 * ```
 *
...
GMap.VectorGrid.Slicer = GMap.VectorGrid.extend({
...
GMap.vectorGrid.slicer = function (geojson, options) {
	return new GMap.VectorGrid.Slicer(geojson, options);
};

GMap.Canvas.Tile = GMap.Canvas.extend({

	initialize: function (tileCoord, tileSize, options) {
		GMap.Canvas.prototype.initialize.call(this, options);
		this._tileCoord = tileCoord;
		this._size = tileSize;
	...
	// add by wusheng
	// 在tile render加入到map的时候，主动执行一次标注绘制，后续通过_update接口来重绘 
	addTo: function(map) {
		this._map = map;
		for (var key in this._layers) {
			var lyr = this._layers[key];
			lyr._gridOffset = this.getOffset();
			this._map.updateLabel(lyr);
		}
	},
	...
	_updateIcon: function (layer) {
		...
		if (img.complete) {
			...
			GMap.DomEvent.on(img, 'load', function() {
				ctx.drawImage(img, p.x, p.y, size.x, size.y);
			});
		}
...
GMap.canvas.tile = function(tileCoord, tileSize, opts){
	return new GMap.Canvas.Tile(tileCoord, tileSize, opts);
};
...
//# sourceMappingURL=GMap.VectorGrid.bundled.js.map
